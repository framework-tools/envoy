...
impl Response {
    ...
    /// # Example:
    /// ```rust
    /// # use envoy::{StatusCode, Response, http::mime};
    ...
    ///
    /// response.set_status(StatusCode::NonAuthoritativeInformation); // or a envoy::StatusCode
    /// assert_eq!(response.status(), StatusCode::NonAuthoritativeInformation);
    /// ```
    /// # Panics
    ...
    /// let error = envoy::http::Error::from(error);
    ///
    ...
    /// Create a `envoy::Response` from a type that can be converted into an
    /// `http_types::Response`.
