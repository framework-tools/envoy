...
#[async_std::test]
async fn test_basic_sessions() -> envoy::Result<()> {
    ...
    app.with(Before(|mut ctx: envoy::Context<()>| async move {
        let visits: usize = ctx.session().get("visits").unwrap_or_default();
        ctx.session_mut().insert("visits", visits + 1).unwrap();
        ctx
    }));
    ...
    let cookie = &cookies["envoy.sid"];
    assert_eq!(cookie.name(), "envoy.sid");
...
#[async_std::test]
async fn test_customized_sessions() -> envoy::Result<()> {
    let mut app = envoy::new();
    app.with(
    ...
    app.at("/nested").get(|req: envoy::Context<()>| async move {
        Ok(format!(
            "/nested {}",
    ...
    assert!(cookies.get("envoy.sid").is_none());
...
#[async_std::test]
async fn test_session_destruction() -> envoy::Result<()> {
    let mut app = envoy::new();
    app.with(SessionMiddleware::new(
        MemoryStore::new(),
        b"12345678901234567890123456789012345",
    ...
    app.with(Before(|mut request: envoy::Context<()>| async move {
    ...
    app.at("/logout")
        .post(|mut req: envoy::Context<()>| async move {
    ...
    assert_eq!(cookies["envoy.sid"].value(), "");
    assert_eq!(cookies.len(), 1);
    Ok(())
}

#[derive(Debug, Clone)]
struct Cookies(Vec<envoy::http::Cookie<'static>>);
impl Cookies {
    fn len(&self) -> usize {
        self.0.len()
    }
    ...
    fn get<'a>(&'a self, name: &str) -> Option<&'a envoy::http::Cookie<'static>> {
        self.0.iter().find(|cookie| cookie.name() == name)
    }
}
impl envoy::http::headers::ToHeaderValues for &Cookies {
